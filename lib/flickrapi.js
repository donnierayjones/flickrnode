...
FlickrAPI.prototype._configure= function(api_key, shared_secret, auth_token) { 
    ...
    this.favorites= new Favorites(this._request);
...
FlickrAPI.prototype.getLoginUrl= function(permissions, frob, callback) {
    if( frob ) {
        ...
        callback(null, "https://flickr.com/services/auth/?api_key="+this.api_key+"&perms="+permissions+"&frob="+frob+"&api_sig="+ sig, frob);
    } else {     
        var self= this;
        this.auth.getFrob(function(err, frob) {
            if( err ) {
                ...
                callback(null, "https://flickr.com/services/auth/?api_key="+self.api_key+"&perms="+permissions+"&frob="+frob+"&api_sig="+ sig, frob)
            }
        });
    }
};
